...
At this point we have one or more sets of DEGs from our experiment. It might be just one set or it might be a large number of comparisons made with a set of DEGs per comparison. This chapter offers some guidance to visualize and - finally - connect some biological relevance to the results.

## Volcano Plot {#volcano}

A volcano plot is often the first visualization of the data once the DEGs are known. This plot shows data for *all* genes and we highlight those genes that are considered DEG by using thresholds for both the (adjusted) p-value and a fold-change.

This scatterplot places the `-log10(pvalue)` values on the y-axis and the log-FC on the x-axis. Coloring is done based on the thresholds (-log10(0.05) for the pvalue and log2(1.75) (±0.8) for the log-FC). Note: the FC-value has been arbitrarily chosen to limit the number of visible DEG's.
...
# Find DEGs (FDR < 0.05 and absolute (non-log)fold change >= 1.75x)
deseq.threshold <- as.factor(abs(deseq.results$log2FoldChange) >= 0.807 & 
...
# Nubmer of DEGs with log-FC >= 0.807
nrow(deseq.degs)
```
...
g = ggplot(data=edgeR.results, 
  ...
  ggtitle("DEGs according to EdgeR with FDR <= 0.05 and log-FC >= 1.75") +
...
g = ggplot(data=deseq.results, 
  ...
  xlab("log2 fold change") + ylab("-log10 p-value") +
  ggtitle("DEGs according to EdgeR with FDR <= 0.05 and absolute FC >= 1.75") +
  # Add the text labels only for DEGs. Using `ggrepel` to make sure labels don't overlap
  geom_text_repel(aes(label=ifelse(padj < 0.05 & abs(log2FoldChange) >= 0.807,
...
gene_list <- topTags(et, n=1000000, sort.by="logFC")
with(gene_list$table, plot(logFC, -log10(FDR), pch=20, main="Volcano plot", xlim=c(-4,4), ylim=c(0,15)))

with(subset(gene_list$table, FDR<.05 ), points(logFC, -log10(FDR), pch=20, col="red"))
with(subset(gene_list$table, abs(logFC)>1), points(logFC, -log10(FDR), pch=20, col="orange"))
with(subset(gene_list$table, FDR<.05 & abs(logFC)>1), points(logFC, -log10(FDR), pch=20, col="green"))
```
...
pander(data.frame(Gene=head(row.names(edgeR.results)),
       ...
       caption = "Comparing FC-values from edgeR, DeSEQ and manually using rlog- and cpm-normalized data")
...
The columns in the heatmap show the four separate knockout (KO) replicates compared to the **average** of the control (WT) replicates. The `KOvsWT` column contains the contents of the `log2FoldChange` data from DeSEQ.
...
pheatmap::pheatmap(KO.fc, cluster_cols = FALSE,
                   main="Heatmap of all DEGs for KO vs WT with an absolute-FC >= 1.75 (DeSEQ)")
```
...
TODO: complete DAVID manual. For now, create a/the file(s) and choose the **Gene Functional Classification** tool in the left-hand menu on the DAVID website and follow instructions.
