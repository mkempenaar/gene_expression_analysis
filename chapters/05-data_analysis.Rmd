...
```{r load_ch6_data, echo=FALSE, eval=FALSE}
rm(list = ls())
load('./data/chapter6.RData')
```

```{r prepare_deseq_data, echo=FALSE}
...
```{r, echo=FALSE, fig.cap="Boxplot comparing spread of log fold change values"}
...
```{r expression_heatmap, echo=FALSE, eval=TRUE, fig.cap="Heatmap showing the expression values of all disomic and trisomic IPSC samples"}
...
```{r lfc_heatmap, echo=FALSE, eval=TRUE, fig.cap="Heatmap showing the Log Fold Changes of all trisomic vs averaged disomic IPSC samples and the average trisomic vs average disomic IPSC samples"}
# Fold Change
...
```{r spia_result_table, echo=FALSE, eval=FALSE}
...
```{r spia_result_plotP, eval=FALSE}
plotP(spia_result)
```
