...
ipsc.tri.vs.di <- as.data.frame(res.ipsc.lfc)[res.ipsc.lfc$padj < 0.05,]
```
...
```{r deseq_results_plotMA, fig.width=10, echo=FALSE}
par(mfrow=c(1,2))
plotMA(res.ipsc, ylim=c(-12,12), main = "IPSC trisomic vs disomic")
plotMA(res.ipsc.lfc, ylim=c(-12,12), main = "IPSC trisomic vs disomic - corrected")
```
**Links**
...
* Gene annotation has been added to the `DGEList` object that is used to run edgeR with the `genes` parameter. This data has been taken from the `AnnotationDbi` package as [shown in Appendix B](#annotationDBI).
* explain BCV-plot
