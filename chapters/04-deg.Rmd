...
To determine whether or not a gene can be classified as a significant DEG we are going to use existing libraries, one of which your own article might refer to. Before we do that we're going to a brief manual examination of 
...
If your data **only** consists of the read counts we are going to apply another normalization technique. The previously used `rlog` normalization is the advised method for the visualizations we performed, but for finding DEGs there are better methods. One of these generally accepted method of normalizing count data is to calculate the *fragments per million mapped fragments* (FPM) value and then transform this with log2. Opposed to FPKM and RPKM this does not include the gene-length in its calculation (which you most likely don't have). If your dataset also includes FPKM or RPKM (pre-normalized), you are allowed to use this data too. Always clearly document if you did so! 
...
* **Assignment**: Search through your article for any advice on how to filter out zero values or low count genes. If there is nothing stated on this subject, think of your own tactic (or search the literature/ online!). It is perfectly fine to discuss with your peers.
    ...
    + Perform the filtering on your dataset. For this you will most likely need to use one of the `apply`, `rowSums`, `rowMeans` functions, combined with maybe the `which`, `all` and `any` functions. Data transformations/ normalizations are often available from functions too, such as the `cpm` function from the `edgeR` library.
...
* **Assignment**: Create a `histogram` of your log2-FC values for *all* genes. Apply the following steps on your *FPM-normalized data* from chapter 3 for at least one comparison (no need to do it for all possible combinations):
  ...
  + calculate the log Fold Change (lFC); simply *subtract* one group (averaged-column) by another, usually `experiment`-`control`
    - Note that since we already have log-transformed data, we do not calculate the ratio (`experiment`/`control`). Also, because of the Logarithm *quotient rule*, we do not divide both average colums as with the ratio calculation, but we subtract: $log_b{(x/y)} = log_b{(x)} - log_b{(y)}$
...
The output of finding DEG's always includes - but is not limited to - a list of *p-values*; one p-value **per gene**. This value indicates whether that gene is a *statistically-significant differentially expressed gene* (SDEG) and to find these genes of interest all we need to do is get all genes with a p-value below our threshold (i.e. `0.05`). Because we show the usage of two different packages below, we *could* get two p-values per gene thus we can make multiple selections, i.e. compare methods, find genes that have a low p-value in both methods and visualize these results in a *venn-diagram* (see section [visualization](#deg-visualization)).
...
* **Assignment**: Perform data-analysis using at least one of the two packages. Alternatively if your article mentioned a different R package for their analysis (for instance the `limma` package) it is allowed to use this one instead. Optionally, use more than one package so we can compare the results between the these packages. 
...
There is no need to normalize the data using the previously used `rlog` or `fpm` functions because the `DESeq2` library will normalize the count data for you as follows:
...
res.ipsc <- results(dds, name = "group_IPSC.trisomic_vs_IPSC.disomic", 
               alpha = 0.05)
# Used throughout chapters 3 and 4
res.ipsc.lfc <- lfcShrink(dds, coef = "group_IPSC.trisomic_vs_IPSC.disomic",
                          type = "apeglm")
res.neuron <- results(dds, name = "group_Neuron.disomic_vs_IPSC.disomic",
               alpha = 0.05)

# Set padjusted NA's to 1
res.ipsc.lfc$padj[is.na(res.ipsc.lfc$padj)] <- 1

ipsc.tri.vs.di <- as.data.frame(res.ipsc.lfc)[res.ipsc.lfc@listData$padj < 0.05,]
```
...
cat('<th align="center">Disomic Neuron vs Disomic IPSC</th></tr>')
cat("<tr><td><pre>")
summary(res.ipsc)
cat("</pre></td>")
cat("<td><pre>")
...
```{r deseqResultsPlotMA, fig.width=10, echo=FALSE, eval=FALSE}
# Disabled; shows different results after rendering
par(mfrow=c(1,2))
plotMA(res.ipsc, ylim=c(-10,10), main = "IPSC trisomic vs disomic")
plotMA(res.ipsc.lfc, ylim=c(-10,10), main = "IPSC trisomic vs disomic - corrected")
```

```{r DESeq2MAPlot, fig.width=8, eval=TRUE, echo=FALSE, fig.cap="MA-plots with the standard DESeq2 output (left) and after shrinking with 'lfcShrink' (right)"}
knitr::include_graphics("chapters/images/DESeq2MAplot.png", dpi = NA)
```
...
qlf <- glmQLFTest(fit, contrast = my.contrats[, "tri_IPSC.vs.di_IPSC"])
