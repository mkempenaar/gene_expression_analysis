...
* and compare these results from those given by using one or more R (Bioconductor) libraries (mostly `edgeR` and `DESeq2`)
...
* **Assignment**: Create a `histogram` of your log2-FC values for *all* genes. Apply the following steps on your *rlog normalized data* from chapter 3 for at least one comparison (no need to do it for all possible combinations):
...
* **Assignment**: Think of a question (i.e. a comparison between two or more sample groups) to ask the data and perform a statistical test to find the relevant genes for this question. Briefly explain this comparison (maybe it was the one resulting in the most interesting results listed in the article) and introduce the test that you are going to perform.
    + Report on the number of significant genes before *and* after [Multiple Testing Correction](#deg-mtc) for each comparison that has been done.

### Students T-test {#deg-ttest}

```{r full_ttest, echo=FALSE}
counts.fpm.dat <- as.data.frame(counts.fpm[,c(di_IPSC, tri_IPSC)])
# Filter for low counts
ipsc.di <- 1:3
ipsc.tri <- 4:6
norm.factors <- colSums(counts.fpm.dat / 1e6)
cpm.data <- as.data.frame(counts.fpm.dat / norm.factors)
# Remove rows where sum(cpm) < 1 *or* more than two zeros
to.keep <- rowSums(cpm.data[ipsc.di]) > 1 & 
  rowSums(cpm.data[ipsc.tri]) > 1 & 
  rowSums(cpm.data == 0) < 2
counts.fpm.dat <- counts.fpm.dat[which(to.keep),]
# Remove low-counts
counts.fpm.dat$pval <- as.numeric(apply(counts.fpm.dat, 1, function(x) {
  t.test(x[ipsc.di], x[ipsc.tri], var.equal = TRUE, alternative = "two.sided")$p.value
}))
counts.fpm.dat$padj <- p.adjust(counts.fpm.dat$pval)
counts.fpm.dat$logFC <- log2(rowMeans(counts.fpm.dat[,ipsc.tri]) / rowMeans(counts.fpm.dat[,ipsc.di]))
degs <- with(counts.fpm.dat, counts.fpm.dat[pval < 0.05,])
```
...
neur.exp <- as.numeric(counts.fpm["KCTD14", di_NEUR])
```

```{r ttest-data, echo=FALSE}
panderOptions('table.split.table', Inf)
set.caption('Data for t-test, di_IPSC (case, 3 samples) vs di_NEUR (control, 3 samples), "KCTD14" gene')
pander(counts.fpm["KCTD14" ,c(di_IPSC, di_NEUR)])
```
...
variance <- var.test(ipsc.exp, neur.exp, alternative = 'two.sided')
```
...
```{r ttest}
test <- t.test(ipsc.exp, neur.exp, alternative = 'two.sided', var.equal = TRUE)
```
...
```{r anovePlot, anova-residuals-normality, fig.cap="ANOVA model plot"}
