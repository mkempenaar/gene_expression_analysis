...
```{block, type='rmdtip'}
**Notes**:
* As a general advice; do not wait with knitting this whole document until the project is done as knitting is very prone to errors and trying to fix these in a large document is not easy
* Give each code chunk the proper attributes, including a name at the minimum. This helps spot errors during knitting as that process mentiones which chunk has been processed. Note that chunk-names *must* be unique
* Make proper use of chapters and sections and include an (optional) table of contents.
```