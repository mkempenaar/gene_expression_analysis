...
Summarize your project proposal on max. 1 A4 and, combined with the slides, are the final deliverables for this chapter (submission through the accompanying BlackBoard course). The difference between the presentation and the written proposal is that the written proposal should contain more technical information that can partially be found in the article:

* List the software and any libraries used for performing the statistical analysis on the count data (i.e. R with bioconductor package(s) DESeq or edgeR).
