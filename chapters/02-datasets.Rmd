...
For week two, you are asked to write short project proposal (max. 2 A4 papers) where you:
...
* Describe what *your* plan is with the dataset, given the choices listed in the section about [Finding Public Data](#finding-public-data). Note that whatever choice you make, the first steps will most likely be the same leading to finding the *Differentially Expressed Genes* (DEGs). It is very difficult to plan your complete project based on what you read in the article and it is not expected to get a planning for the full remaining 6 weeks. You can however specify a bit more given what you've read in the article, such as trying to reproduce a certain figure or repeat a certain analysis step.
* List the software and any libraries used in the article for performing the statistical analysis on the count data (i.e. R with bioconductor package(s) DESeq or edgeR).
* Other software or methods described for downstream analysis of the DEGs found (i.e. 'Gene Ontology enrichment analysis using DAVID')
* An example (first 5 rows) and very brief description of the available data. Sometimes experiments include much more then just a single count-column but also annotation data might be present.