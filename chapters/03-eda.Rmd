...
```{r GEO_RNA-Seq, echo=FALSE, fig.height=4, fig.cap="Finding an experiment on GEO with a TXT file as supplementary data"}
...
## Visualizing using `summary`, `boxplot`, `scatterplot` & `MA-plot` {#EDA_part1}
...
Also note that these steps are just a selection. Furthermore, make sure that for every visualization you make, add proper axis-labels containing the measurement units (important!). The following three plots are based on [GSE74329](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE74329) titled *Transcriptome anlaysis of gastrointestinal tract of pre-weaned cavles*. As you might be able to see, all values are *log-transformed* using the `log2` because very often the numerical values have a very high range which will 'hide' the details on the plots. See the section about the [Fold Change value](#deg-fc) in chapter 4 for further details. It is fine to use non-log-transformed (simply the raw-)data, otherwise use for instance `boxplot(log2(dataset))` for plotting.

> **Note**: instead of using the basic R-plotting library (i.e. `plot`, `boxplot`, `hist`, etc.) you can also opt for using the (challenging) [`ggplot2`](http://ggplot2.tidyverse.org/index.html) library that is also used for the following three figures. While constructing a `ggplot2` plot feels like learning yet another language, there are many resources available online that you can follow.

### Statistics {#eda_statistics}

Even the most basic statistics can give some insight into the data such as performing a 6-number-statistic on the data **columns** using the `summary()` function. Note: you can also use the `pander` function to *pretty-print* a summary from a markdown document.
...
![*An MA-plot showing the average normalized count values (**A**, x-axis) vs. the log2 fold-change (**M**, y-axis).*](chapters/images/RNAmaplot_ggplot2.png)
