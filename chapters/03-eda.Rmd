...
The following code shows how to load the two available files containing the raw-count data (one file per 6 samples), stored locally in the `data/gse101942/` folder. Some simple 'cleanup' steps are performed which aren't required by helpful for demonstration purposes (i.e., renaming samples to identify groups).
...
ipsc <- read.table('./data/gse101942/GSE101942_IPSC_rawCounts.txt', header = TRUE)
neuron <- read.table('./data/gse101942/GSE101942_Neuron_rawCounts.txt', header=TRUE)
# Merge by rowname
counts <- merge(ipsc, neuron, by = 0, all.x = TRUE, all.y = TRUE, sort = FALSE)
...
ggplot(data=data.frame(read_depth, A1BG=as.numeric(counts[3,])), aes(x=names(counts),y=read_depth, fill=exp)) + 
  ...
  ggtitle("Read counts for GSE74329") +
...
pander(head(rld), caption = 'Table 2; RLog transformed count data')
```
...
pander(.dist.mat[1:3, 1:3], caption = 'Table 3; Sample distances for the trisomic IPSCs (Euclidean method)')
```
...
```{r pca-plot, echo=TRUE, eval=TRUE}
...
**Note**: Once you are satisfied with the data and/ or reported on your findings, we do not return to any of these steps in the next chapters. If there are mentions of creating a heatmap for instance, this means a heatmap of *expression data* instead of the *sample distances* we've shown in this chapter.
