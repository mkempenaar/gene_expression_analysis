...
```{block, type='rmdtip'}
Note the order of axis in the `lm` function. The arguments are in a `formula` denoted `y` *over* `x`. Make sure the order is correct as the `plot` function takes the order of `plot(x, y)`.
```
...
```{r RNAScatterplot, fig.width=8, eval=TRUE, echo=FALSE, fig.cap="A simple scatterplot showing the correlation between two samples or experiments (see the text above). This image shows the values log2 transformed which in most cases gives a clearer picture."}
knitr::include_graphics("chapters/images/RNAscatterplot_ggplot2.png", dpi = NA)
```
...
```{r MAPlotBias, fig.width=8, eval=TRUE, echo=FALSE, fig.cap="An MA-plot showing the average count values (**A**, x-axis) vs. the log2 fold-change (**M**, y-axis) with a very clear bias (data not centered around y=0 line)."}
knitr::include_graphics("chapters/images/RNAmaplot_bias.jpg", dpi = NA)
```
...
```{r MAPlot, fig.width=8, eval=TRUE, echo=FALSE, fig.cap="An MA-plot showing the average count values (**A**, x-axis) vs. the log2 fold-change (**M**, y-axis)."}
knitr::include_graphics("chapters/images/RNAmaplot_ggplot2.png", dpi = NA)
```

```{block, type='rmdtip'}
As for the scatterplot, select one or two sets of two samples to create MA-plots for. Base this selection on what you see in for instance the boxplot or density plot and show one set of similar samples and one set of deviating samples.
```
...
```{r .pre-processing_example_data, echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE}
library(DESeq2)
...
set.caption('Table 1; Mapped reads per sample (millions)')
pander(read_depth)
...
pander(head(rld[ipsc.di]), caption = 'Table 2; RLog transformed disomic IPSC count data')
```
...
pander(.dist.mat[ipsc.di, 1:3], caption = 'Table 3; Sample distances for the disomic IPSCs (Euclidean method)')
```
...
```{r pcaPlot, echo=TRUE, eval=TRUE, fig.cap="Principal Component Analysis for all samples"}
...
The resulting figure is a near optimal situation, where each sample is clustered with its replicates. It often happens that samples are all mixed which can be fine too (we're looking at comparisons of 20.000+ genes reduced to a single number), but keep an eye on any outliers.
