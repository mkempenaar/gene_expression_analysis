...
* Examine the samples included in the experiment and create as many R character objects as needed to store the classification.
    ...
    + These variables allows us to repeatedly access the same data during the rest of this course, i.e. the code `boxplot(counts[case])` immediately shows that we are plotting the `case` samples from the `counts` data which is more clear than reading `boxplot(counts[c(5:8)])` as that gives no reference to what these indices mean.
...
To use the `vst` function from the `DESeq2` library we must contruct a `DESeqDataSet`-object consisting of the count data combined with sample annotation. Since we only want to use it (for now) for performing an vst-transformation we use the most basic form with a very simple `design` and the sample names as annotation (the `colData` argument):
